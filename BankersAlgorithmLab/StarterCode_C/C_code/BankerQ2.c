...
int checkSafe(int customerIndex, int *request)
{
    ...
    // initialise the finish to be false
		...
		for (int i = 0; i < numberOfCustomers; i++)
		{
			...
			if (finish[i] == 0)
			{
				for (int j = 0; j < numberOfResources; j++)
				{
					...
					{
						countResourcesSatisfied++;
					}
					if (countResourcesSatisfied == numberOfResources)
					{
						possible = 1;
						for (int j = 0; j < numberOfResources; j++)
						{
